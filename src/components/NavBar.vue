                    ...
                    <Popper arrow class="inline-block">
                        ...
                        <template #content="{}">
                            <div class="menu">
                                ...
                                >{{nickname}}</h3>
                                <ul class="">
                                    <li class="list-none py-1 flex justify-start hover:text-black">
                                        <box-icon name="user" animation="burst-hover">
                                            <i class="bx bx-user"></i>
                                        </box-icon>
                                        <router-link
                                            :to="{ name: 'Profile' }"
                                        ><span class="pl-2  transition-all duration-500">Profile</span></router-link>
                                    </li>
                                    <li class="list-none py-1 border-t border-gray flex justify-start hover:text-black">
                                        <box-icon name="heart" animation="flashing-hover">
                                            <i class="bx bx-heart"></i>
                                        </box-icon>
                                        <router-link
                                            :to="{ name: 'Favorites' }"
                                        ><span class="pl-2  transition-all duration-500">Favorites</span></router-link>
                                    </li>
                                    <li class="list-none py-2 border-t border-gray flex justify-start hover:text-black">
                                        <box-icon name="log-out-circle" animation="fade-left-hover">
                                            <i class="bx bx-log-out-circle"></i>
                                        </box-icon>
                                        <span class="pl-2 cursor-pointer transition-all duration-500" @click="logout">Logout</span>
...
const settingProfile =() => {
    // console.log(localStorage.user, 'this is local storage')
    if(localStorage.user){
        // console.log('what the hack authenitacted')
        nickname.value = JSON.parse(localStorage.user).nickname
        email.value = JSON.parse(localStorage.user).email
        avatar.value = JSON.parse(localStorage.user).picture
    }
}

settingProfile()
console.log(authenticated.value, 'authentication value')
